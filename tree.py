...
class BinaryTree:
    ...
    def delete(self, value: Any):
        ...
        deleter_other_values = None
        if self.root is not None:
            self.root, delete_value, deleter_other_values = __delete(
                self.root, value)
    ...
    def by_level(self):
        tree_queue = Cola()
        if self.root is not None:
            tree_queue.encolar(self.root)

            while tree_queue.tamanio() > 0:
                node = tree_queue.desencolar()
                print(node.value, node.other_values)
                if node.left is not None:
                    tree_queue.encolar(node.left)
                if node.right is not None:
                    tree_queue.encolar(node.right)
    ...
    def contar_repeticiones(self, value: Any):
        def __contar_repeticiones(root):
            if root is None:
                return 0
            contador = 1 if root.value == value else 0
            contador += __contar_repeticiones(root.left)
            contador += __contar_repeticiones(root.right)
